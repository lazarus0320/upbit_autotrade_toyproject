...
class OrderbookWidget(QWidget): ## 위젯 받아와서 UI를 띄우는 클래스
    def __init__(self, parent=None, ticker="KRW-BTC"):  # 새창으로 BTC에 대한 호가창 위젯을 띄움
        super().__init__(parent)
        uic.loadUi("resource/orderbook.ui", self)
        self.ticker = ticker 
        self.ow = OrderbookWorker(self.ticker) # 스레드의 객체 ow 선언
    ...
    def updateData(self, data):     ## 데이터를 받아서 화면에 업데이트
        ...
        for i in range(0,15):   # api로 얻어온 호가창 개수는 매수, 매도 각각 15개
            ...
            bid_Price_List.append(int(data['orderbook_units'][i]['bid_price'])) # 각 호가창의 매수 호가 가격들을 바인딩
            ...
            ask_Price_List.append(int(data['orderbook_units'][i]['ask_price']))
        ...
        for i, v in enumerate(ask_Price_List):   #  enumerate -> 인덱스 번호와 컬렉션의 원소를 tuple형태로 반환 따라서 i는 인덱스번호, v는 그 값이 된다.
            ...
            item_0.setText(str('{:0,.0f}'.format(v)))   # 호가 출력. 000,000,000형태로 포멧팅.
            ...
            item_2.setRange(0, max_Trade_Price) #   3번째 열의 모든 셀들의 범위를 0~전체 매도+매수금 총액의 최댓값으로 지정. 따라서 100%는 전체 매도+매수금의 총액
            item_2.setFormat(f"{sum_Ask_Price[i]:,}")   # 매도 총액을 000,000,000형태로 포멧팅.
            # 각 호가번호의 매도 금액 총액을 어떻게 표시할건지를 정함. 여기서는 숫자에 ,을 삽입
            item_2.setValue(sum_Ask_Price[i])    # setValue() : 진행표시줄의 진행 상태를 특정 값으로 설정. 매도 총액을 출력.
        ...
        for i, v in enumerate(bid_Price_List):   #  처음부터 끝까지 1칸 간격으로. enumerate -> 인덱스 번호와 컬렉션의 원소를 tuple형태로 반환 따라서 i는 인덱스번호, v는 그 값이 된다.
            ...
            item_0.setText(str('{:0,.0f}'.format(v)))   # 매수 호가 출력
                # 수량 출력
            ...
            item_2.setValue(sum_Bid_Price_List[i])   # setValue() : 진행표시줄의 진행 상태를 특정 값으로 설정. 매수 총액을 출력
